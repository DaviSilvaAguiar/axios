...
<title>RDC #{{ str_pad($caixa->id, 6, '0', STR_PAD_LEFT) }}</title>
<style>
...
@php
  ...
  $total = $caixa->despesas->sum('valor');
  $hash  = strtoupper(substr(sha1('rdc-' . $caixa->id . '-' . $caixa->created_at), 0, 32));

  $todosAnexos = [];
  foreach ($caixa->despesas as $i => $despesa) {
    ...
    <tr>
      <td class="doc-brand" style="width:65%">
        ...
        <div class="doc-brand-sub">
          RELATÓRIO DE DESPESAS DE CAIXA (RDC)<br>
      ...
      <td class="doc-number-block" style="width:35%">
        ...
        <div class="doc-number-value">{{ str_pad($caixa->id, 6, '0', STR_PAD_LEFT) }}</div>
        <div class="doc-number-tipo">PRESTAÇÃO DE CONTAS</div>
      </td>
    </tr>
  </table>
</div>
...
<table class="status-banner">
  <tr>
    ...
    <td class="status-banner-date" style="text-align:right;">EMISSÃO: {{ $caixa->created_at?->format('d/m/Y H:i') }}</td>
  </tr>
</table>
...
<div class="section">
  ...
  <div class="section-body">
    <table class="fields-table">
      <tr>
        <td style="width:60%">
          <span class="field-label">DESCRIÇÃO DA PRESTAÇÃO</span>
          <span class="field-value">{{ $caixa->descricao }}</span>
        </td>
        <td style="width:20%">
          <span class="field-label">INÍCIO PERÍODO</span>
          <span class="field-value">{{ $caixa->data_inicio_periodo?->format('d/m/Y') ?? '—' }}</span>
        </td>
        <td style="width:20%">
          <span class="field-label">FIM PERÍODO</span>
          <span class="field-value">{{ $caixa->data_fim_periodo?->format('d/m/Y') ?? '—' }}</span>
        </td>
      </tr>
    </table>
  </div>
...
<div class="section">
  ...
  <div class="section-body">
    <table class="fields-table">
      <tr>
        <td style="width:55%">
          <span class="field-label">NOME DO REQUISITANTE</span>
          <span class="field-value">{{ mb_strtoupper($caixa->descricao_requisitante ?: ($caixa->usuarioRequisitante->nome ?? 'NÃO INFORMADO')) }}</span>
        </td>
        <td style="width:45%">
          <span class="field-label">CPF / CNPJ</span>
          <span class="field-value">{{ $caixa->cpf_cnpj_requisitante ?: '—' }}</span>
        </td>
      </tr>
      <tr>
        <td style="width:55%">
          <span class="field-label">SETOR</span>
          <span class="field-value">{{ mb_strtoupper($caixa->setor_requisitante ?: '—') }}</span>
        </td>
        <td style="width:45%">
          <span class="field-label">CENTRO DE CUSTO</span>
          <span class="field-value">{{ mb_strtoupper($caixa->centroDeCusto?->descricao ?? '—') }}</span>
        </td>
      </tr>
    </table>
  </div>
...
<div class="section">
  ...
  <div class="section-body">
    <table class="expense-table">
      ...
      <tbody>
        @forelse($caixa->despesas as $i => $despesa)
          <tr>
      ...
      @if($caixa->despesas->isNotEmpty())
      <tfoot>
        <tr class="subtotal-row">
          <td colspan="5" class="right" style="padding-right:10px;">
            TOTAL DE ITENS: {{ $caixa->despesas->count() }}
          </td>
    ...
    @if($caixa->obs)
      <div class="alert-box">
        <strong>OBSERVAÇÕES:</strong> {{ mb_strtoupper($caixa->obs) }}
      </div>
    @endif
  </div>
</div>
...
{{-- ═══ DADOS BANCÁRIOS ═══ --}}
@if($caixa->banco || $caixa->chave_pix || $caixa->agencia || $caixa->numero_banco)
<div class="section">
    ...
    <table class="fields-table">
      <tr>
        <td style="width:25%">
          ...
          <span class="field-value">{{ mb_strtoupper($caixa->banco ?: '—') }}</span>
        </td>
        <td style="width:15%">
          <span class="field-label">AGÊNCIA</span>
          <span class="field-value">{{ $caixa->agencia ?: '—' }}</span>
        </td>
        <td style="width:25%">
          <span class="field-label">CONTA</span>
          <span class="field-value">{{ $caixa->numero_banco ?: '—' }}</span>
        </td>
        <td style="width:35%">
          <span class="field-label">CHAVE PIX</span>
          <span class="field-value">{{ $caixa->chave_pix ?: '—' }}</span>
        </td>
      </tr>
    </table>
  </div>
...
<table class="footer">
  <tr>
    ...
    <td class="footer-right">
      DOCUMENTO ID: {{ str_pad($caixa->id, 8, '0', STR_PAD_LEFT) }}<br>
