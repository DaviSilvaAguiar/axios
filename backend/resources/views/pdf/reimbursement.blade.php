...
<title>Reimbursement #{{ str_pad($reimbursement->id, 6, '0', STR_PAD_LEFT) }}</title>
<style>
  ...
  <table class="doc-header-inner">
    <tr>
      ...
      <td class="doc-number-block" style="width:35%">
        ...
        <div class="doc-number-value">{{ str_pad($reimbursement->id, 6, '0', STR_PAD_LEFT) }}</div>
        <div class="doc-number-tipo">REIMBURSEMENT</div>
      </td>
    </tr>
  </table>
</div>
...
<table class="status-banner">
  <tr>
    ...
    <td class="status-banner-date" style="text-align:right;">ISSUED: {{ $reimbursement->created_at?->format('d/m/Y H:i') }}</td>
  </tr>
</table>
    ...
    <table class="fields-table">
      <tr>
        <td style="width:60%">
          ...
          <span class="field-value">{{ $reimbursement->title }}</span>
        </td>
        <td style="width:20%">
          <span class="field-label">PERIOD START</span>
          <span class="field-value">{{ $reimbursement->period_start_date?->format('d/m/Y') }}</span>
        </td>
        <td style="width:20%">
          <span class="field-label">PERIOD END</span>
          <span class="field-value">{{ $reimbursement->period_end_date?->format('d/m/Y') }}</span>
        </td>
      </tr>
    </table>
  </div>
    ...
    <table class="fields-table">
      <tr>
        <td style="width:55%">
          ...
          <span class="field-value">{{ mb_strtoupper($reimbursement->requester_name ?? $reimbursement->user->name ?? 'NOT PROVIDED') }}</span>
        </td>
        <td style="width:45%">
          <span class="field-label">TAX ID</span>
          <span class="field-value">{{ $reimbursement->requester_tax_id ?: '—' }}</span>
        </td>
      </tr>
      <tr>
        <td style="width:55%">
          <span class="field-label">CORPORATE EMAIL</span>
          <span class="field-value">{{ $reimbursement->user->email ?? '—' }}</span>
        </td>
        <td style="width:45%">
          <span class="field-label">ACCESS ROLE</span>
          <span class="field-value">
            @switch($reimbursement->user?->role)
              @case(1) ADMIN @break
              @case(2) AUDITOR @break
              @case(3) PROVIDER @break
              @default — @break
            @endswitch
          </span>
        </td>
...
<div class="section">
  ...
  <div class="section-body">
    <table class="expense-table">
      ...
      <tbody>
        @forelse($reimbursement->items as $i => $item)
          <tr>
      ...
      @if($reimbursement->items->isNotEmpty())
      <tfoot>
    ...
    @if($reimbursement->status === 7 && $reimbursement->rejection_reason)
      <div class="alert-box">
        <strong>REJECTION REASON:</strong> {{ mb_strtoupper($reimbursement->rejection_reason) }}
      </div>
    @endif

    @if($reimbursement->status === 5 && $reimbursement->scheduled_payment_date)
      <div class="alert-box">
        <strong>SCHEDULED PAYMENT:</strong> {{ $reimbursement->scheduled_payment_date->format('d/m/Y') }}
      </div>
    @endif
  </div>
</div>

@if($reimbursement->bank || $reimbursement->pix_key || $reimbursement->branch || $reimbursement->account_number)
<div class="section">
    ...
    <table class="fields-table">
      <tr>
        <td style="width:25%">
          ...
          <span class="field-value">{{ mb_strtoupper($reimbursement->bank ?: '—') }}</span>
        </td>
        <td style="width:15%">
          <span class="field-label">BRANCH</span>
          <span class="field-value">{{ $reimbursement->branch ?: '—' }}</span>
        </td>
        <td style="width:25%">
          <span class="field-label">ACCOUNT</span>
          <span class="field-value">{{ $reimbursement->account_number ?: '—' }}</span>
        </td>
        <td style="width:35%">
          <span class="field-label">PIX KEY</span>
          <span class="field-value">{{ $reimbursement->pix_key ?: '—' }}</span>
        </td>
      </tr>
    </table>
  </div>
...
<table class="footer">
  <tr>
    ...
    <td class="footer-right">
      DOCUMENT ID: {{ str_pad($reimbursement->id, 8, '0', STR_PAD_LEFT) }}<br>
