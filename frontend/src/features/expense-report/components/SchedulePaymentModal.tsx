...
export default function SchedulePaymentModal({ onConfirm, onCancel }: Props) {
  const {
    control,
    handleSubmit,
  ...
  async function onSubmit(data: FormData) {
    await onConfirm(data.payment_date);
  }
          ...
          <div className="mb-4 flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-body-sm text-app-text-muted">
                Enter the expected payment date for this report.
              </p>
            </div>
            <button
              onClick={onCancel}
              className="ml-4 rounded-full p-1 text-app-text-muted hover:bg-app-hover cursor-pointer"
            >
              <X size={20} />
            </button>
          </div>
